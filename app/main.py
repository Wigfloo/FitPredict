...
try:
    ruta_modelo_5k = f'models/modelo_5k.pkl'
    model_carrera_5k = pickle.load(open(ruta_modelo_5k, 'rb'))

    ruta_modelo_10k = f'models/modelo_10k.pkl'
    model_carrera_10k = pickle.load(open(ruta_modelo_10k, 'rb'))

    ruta_scaler_carrera = f'models/scaler_3k_pred.pkl'
                    ...
                    with st.spinner("Preprocesando datos y prediciendo..."):
                        ...
                        if predicted_profile:
                            ...
                            if perfil_normalizado == "elite":
                                ...
                                st.markdown(
                                    "### 📋 Plan de entrenamiento (Novato)")
                                st.dataframe(entrenamiento_novato)

                            else:
                                st.info(
                                    "🔍 No se pudo determinar un perfil válido para mostrar un plan de entrenamiento.")
                        else:
                            st.warning(
